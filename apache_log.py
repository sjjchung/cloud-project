...
dayGroupedHosts = dayToHostPairTuple.distinct().groupByKey()

dayHostCount = dayGroupedHosts.map(lambda PT : (PT[0],len(PT[1])))
...
avgDailyReqPerHost = (sortedByDay.map(lambda PT: (PT[0],len(PT[1])/len(set(PT[1]))))).cache()
...
badEndpoints = badRecords.map(lambda x : x.host)
...
errDateCountPairTuple = badRecords.filter(lambda x:x.response_code == 404).map(lambda x: (x.date_time.day, 1))
